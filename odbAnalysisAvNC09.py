...
for p in range(10):
	...
	for k in range(1):
		...
		# for j in range(l0,lf1):
			...
			MaxPrinc=[]
			DirVec=[]
			pr=open('RailprincStress-'+str(j)+'.txt','w')
			...
			for i in range(0,len(railStress.values)):
				...
				vonMises.append(railStress.values[i].mises)
				...
				# Princstr.append([evals[0],evals[1],evals[2]]) 
				# PrincStr[i]=dict({"Max. princ.":maxpr,"Direction":DirVec})
				s33.append(astress)
				MaxPrinc.append(maxpr)
				[pr.write(str(evals[i])+'\t') for i in range(3)]
				[pr.write(str(evecs[i])[0]+'\t') for i in range(3)]
				[pr.write(str(evecs[i])[1]+'\t') for i in range(3)]
				[pr.write(str(evecs[i])[2]+'\t') for i in range(3)]
				pr.write('\n')
				# f.write(str(astress)+'\n')
			...
			#computing effective  and mean stress
